...
site = np.random.choice(
    ["Site A", "Site B", "Site C", "Site D", "Site E"], N, p=site_probs
)
...
enrollment_date = [
    start_date + timedelta(days=int(x)) for x in np.random.uniform(0, 730, N)
]
enrollment_date_str = [d.strftime("%Y-%m-%d") for d in enrollment_date]

# Intervention group: balanced randomization
intervention_group = np.random.choice(
    ["Control", "Treatment A", "Treatment B"], N, p=[0.33, 0.33, 0.34]
)
...
follow_up_months = (
    np.clip(np.array(days_since_enrollment) / 30, 0, 24).round(0).astype(int)
)
...
cholesterol_ldl = (
    np.clip(cholesterol_total - cholesterol_hdl - cholesterol_total * 0.2, 40, 250)
    .round(0)
    .astype(int)
)
...
cvd_risk = (
    (1 - cv_health) * 0.3
    + (age - 40) / 200
    - treatment_a_effect * 0.1
    - treatment_b_effect * 0.05
)
...
adherence_pct_base = (
    engagement * 85 + site_quality_values * 10 + np.random.normal(0, 10, N)
)
...
dropout_risk = (
    (1 - engagement) * 0.3 + (adverse_events > 2) * 0.2 + (follow_up_months > 18) * 0.1
)
...
print(
    f"  Missing cholesterol: {df['cholesterol_total'].isna().sum()} ({df['cholesterol_total'].isna().mean() * 100:.1f}%)"
)
