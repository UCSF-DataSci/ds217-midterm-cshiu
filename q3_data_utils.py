...
def clean_data(
    ...
    df_clean = df_clean.replace(sentinel_value, pd.NA)
    return df_clean
...
def fill_missing(df: pd.DataFrame, column: str, strategy: str = "mean") -> pd.DataFrame:
    ...
    if strategy == "mean":
        ...
        raise ValueError(
            f"Unknown strategy: {strategy}. Use 'mean', 'median', or 'ffill'."
        )
    return df_filled
...
def filter_data(df: pd.DataFrame, filters: list) -> pd.DataFrame:
    ...
    for filter_cat in filters:
        column = filter_cat["column"]
        condition = filter_cat["condition"]
        ...
        if condition == "equals":
            ...
            # value should be a list [min, max]
...
def transform_types(df: pd.DataFrame, type_map: dict) -> pd.DataFrame:
    ...
    for column, target_type in type_map.items():
        ...
        if target_type == "datetime":
            ...
            raise ValueError(
                f"Unknown type: {target_type}. Use 'datetime', 'numeric', 'category', or 'string'."
            )
    return df_typed
